...
- `mnaipro operator` is the thin launcher on top of the stable `mnaipro` surfaces; it recommends the next stable command from the current health evidence and can optionally execute that command with `--run`, preserving explicit bridge and vault context when it launches the child command. Phase 8 optional expansion is complete once this launcher and its docs/smoke coverage are in place.
