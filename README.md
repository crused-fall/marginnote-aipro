...
When the bridge falls back locally, the compact `mnaipro doctor --compact`, `mnaipro status --compact`, `mnaipro bridge status --compact`, and `mnaipro overview --compact` forms also surface `bridge_offline_reason=...` so the fallback cause stays visible without opening JSON.
