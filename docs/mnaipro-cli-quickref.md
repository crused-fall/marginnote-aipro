...
When the bridge falls back locally, `mnaipro status --compact`, `mnaipro bridge status --compact`, and `mnaipro overview --compact` also surface `bridge_offline_reason=...`, so the fallback cause stays visible without opening JSON.
